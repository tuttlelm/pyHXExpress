...
def group_fits(fitsdf,groups,max_sd = 0.5,min_pop = 0.05,z='50'):
    ...
    for sample in fitsdf['sample'].unique():
        for group,v in groups.items():
            ...
            p1_sd = ((pop1 / (use_data['RFU_pop2'] - use_data['RFU_pop1']) )**2) * use_data['pop1_sd']**2 + \
                    ((use_data['RFU_avg'] - use_data['RFU_pop1'])/(use_data['RFU_pop2'] - use_data['RFU_pop1'])**2) * use_data['pop2_sd']**2 + \
                    ((1/(use_data['RFU_pop2'] - use_data['RFU_pop1']))**2) * use_data['avg_sd']**2      #partial derivatives for error propogation
